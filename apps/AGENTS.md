...
狀態：四個主目的地 Home / Inbox / Explore / AI 已作為 current app-shell 契約落地；`/{login}` 仍是 canonical User／Organization locator，`/profile` 是 authenticated viewer 的 secondary Profile hub composition，不是第二個 identity locator。Home header avatar 仍可直達 canonical `/{login}`，Rich Menu「個人」進 `/profile`；兩者都不佔 Bottom Navigation。其餘 GitHub Mobile pattern 仍只在本地 owner/contract 真實存在時逐 slice 採用。文件或 benchmark 出現一個上游 UI 項目，不代表本產品已具備該 capability。
