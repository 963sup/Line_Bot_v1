...
| 探索 Explore | `/explore` | Repository-owned discovery composition：`/trending` 的 7-day active-Star ranking + Awesome Lists entry + current-access-filtered Issue Activity；Awesome Lists 只是 public Repository Star List discovery vocabulary，不建立第二 owner |
| AI | `/assistant` | 既有 Assistant one-shot Ask / Issue-draft Generate / text Review；不建立 durable chat session 或 autonomous writer |

Current shell 固定四個主目的地與 Bottom Navigation；Account/Profile avatar 只出現在 Home header，使用 Account-owned current login 導向 canonical `/{login}` Profile。若既有 Account projection 屬於 legacy User 且尚無 login，avatar 僅可導向 `/settings/profile` 讓 User 明確建立 locator；不得從 LINE profile、provider metadata、UserId 或 display name 推導 login。`/profile` 是 authenticated viewer 的 Profile hub，可組合本人 Account/Repository/Organization 等 owner-approved projection，但不是第二個 User locator；分享仍使用 canonical `/{login}`。Viewer自己的 canonical User Profile右上角才顯示 Settings齒輪並導向 `/settings`；其他 User/Organization Profile不得因此取得 viewer設定入口。Inbox、Explore、AI 與其他工作頁不重複顯示 Profile avatar；`/settings` 是 viewer configuration secondary surface，Profile 與 Settings 不再混用。Home current IA固定為 `My Work → Favorites → Shortcuts → Recent`：My Work 的 Issues/Discussions只做 Repository selector gateway，不冒充跨 Repository feed；Projects在沒有 runtime contract前只顯示 target；Favorites只重用 Repository Star projection；Shortcuts只放固定現有 route，不持久化 saved shortcuts；Recent沒有 owner/runtime時只顯示 empty state，不從 localStorage/browser history或其他 owner activity造資料。`/repositories` 保留為 Home / Explore 可進入的 Repository collection/workbench 與既有 deep link，不是第五個 global tab。Inbox 是 presentation vocabulary，`/notifications` 與 `/api/notifications` 是 current published transport；兩者不要求同名，也不能因 tab 命名順便建立 `packages/inbox`、平行 schema 或新的 source root。
