...
- Explore current IA is `Discover → Trending Repositories / Awesome Lists → Activity`；`/explore/trending` 是 Trending 的 dedicated child surface，返回 `/explore` 並維持 Explore tab。Trending、Activity 與 Awesome Lists 都消費 Repository owner contracts：Awesome Lists 是 active-owner public Repository Star Lists 的 presentation vocabulary，只顯示 viewer 當下可見的 Repository/count；不建立 Explore/List 第二 authority、cross-owner feed 或 recommendation score。
...
- `/profile` is the authenticated viewer Profile hub composition. It may combine Account Profile/Achievement/Activity, Repository and Organization owner projections through their published transports; unavailable owners render unavailable/empty states instead of fake counts. `/{login}` remains the canonical User/Organization locator and `/profile` never authorizes by itself.\n- `/settings` is viewer Account configuration only; Enterprise, Organization, Team, operational utilities and Admin keep their own owner surfaces instead of being grouped under Settings.
