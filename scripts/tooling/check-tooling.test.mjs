...
test("validation entrypoints stay canonical and read-only", (t) => {
  ...
  write("package.json", JSON.stringify(packageJson));
  for (const command of ["schema:local", "schema:remote", "line:rich-menu", "format"]) {
...
test("Release routes only schema-state changes through one reconciliation pass before publication", (t) => {
  ...
  rejects(root, "current-main/sync/evidence");

  write(
    ".github/workflows/release.yml",
    workflow.replace("pnpm schema:remote sync", "pnpm schema:remote sync --allow-destructive"),
  );
  rejects(root, "must never authorize destructive reconciliation");
...
test("Supabase Replace is the only explicit destructive workflow", (t) => {
  const { root, write } = fixture(t);
  ...
  rejects(root, "validate/deployed-Web/prepare/replace/evidence");
