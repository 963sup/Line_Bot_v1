...
test("Release routes schema and workflow changes through one reconciliation pass before publication", (t) => {
  ...
  write(
    ...
    workflow.replace(
      "run: pnpm schema:remote prepare\n        env: {}",
      "run: pnpm schema:remote prepare\n        env:\n          SUPABASE_LEGACY_ENTERPRISE_NAME: forbidden",
    ),
  );
  rejects(root, "must not own explicit Enterprise business metadata");
  ...
  write(
    ".github/workflows/release.yml",
    workflow.replace("pnpm schema:remote sync", "pnpm schema:remote sync --allow-manual"),
  );
  rejects(root, "never authorize manual");
...
test("manual Supabase reconciliation binds apply to reviewed plan and shared resource lock", (t) => {
  ...
  rejects(root, "explicit legacy Enterprise name/slug");
