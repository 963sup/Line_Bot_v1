...
2. 完成 repository validation，再依 runtime dependency order 放行外部 contract；validation、database convergence、deployment 與 provider publication 是不同 evidence，但需要彼此相容的 revision 必須由同一 Release ordering 約束。
   ...
   - 每個 validated main 的 Supabase job 先執行 metadata-free additive `repair`，只復原可由 current declarative source 完整決定、且不需要猜 business metadata 的 runtime compatibility；這層 recovery 不等於 full convergence。若 declarative schema 受影響，再執行 preserve-data `prepare`（只接受 explicit business metadata）與 history-free plain `sync`；若 plan 分類為 `manual`，automatic Release 必須 fail closed，改由 manual reconciliation 的 `prepare-plan → review SHA-256 → apply` 承接 exact-plan authorization與 recovery-readiness attestation。若 declarative schema 未變，`repair` 後仍執行 remote `verify`。只有同一 validated SHA 的 schema diff/readback 與 migration-history fingerprint成立後，database contract 才算 converged。
