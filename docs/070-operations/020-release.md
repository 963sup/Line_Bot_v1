...
2. 完成 repository validation，再依 runtime dependency order 放行外部 contract；validation、database convergence、deployment 與 provider publication 是不同 evidence，但需要彼此相容的 revision 必須由同一 Release ordering 約束。
   ...
   - 每個 validated main 的 Supabase job 先執行 metadata-free additive `repair`，只復原可由 current declarative source 完整決定、且不需要猜 business metadata 的 runtime compatibility；這層 recovery 不等於 full convergence。若 declarative schema 受影響，再執行 preserve-data `prepare`（只接受 explicit business metadata）與 history-free plain `sync`；若 plan 分類為 `manual`，automatic Release 必須 fail closed，改由 manual reconciliation 的 `prepare-plan → retained provenance artifact → review → apply` 承接 exact-plan authorization與 recovery-readiness attestation。Apply 必須證明 reviewed artifact來自同一 current source SHA 的成功 prepare-plan run，並保存 non-secret recovery evidence reference；單獨貼一個 SHA-256 不構成完整 authorization provenance。若 declarative schema 未變，`repair` 後仍執行 remote `verify`。只有同一 validated SHA 的 schema diff/readback 與 migration-history fingerprint成立後，database contract 才算 converged。
