...
Rich Menu desired-state source 是 release-owned external state。當 `assets/line/rich-menu/**`、`definition.ts` 或 `desired-state.server.ts` 的變更合併到 `main`，GitHub `Release` 才進入 LINE publication path；它先要求同一 SHA 的 repository `Validate` 與 `mini-app-line` Vercel deployment 成功，再做 repository-only preview，最後以單一 `publish all` process 重新執行 LINE preflight／create／upload／activate／readback。沒有 Rich Menu desired-state change 時不配置 LINE publication job。LINE secret `LINE_CHANNEL_ACCESS_TOKEN` 只注入 publish step；Current-stage MINI App URL 仍由 repository source 擁有，不把公開 LIFF / Login Channel identity 重複存成 secret，也不讀 Attendance／Supabase runtime configuration。
