...
若 declarative schema 相對 cursor 有變更，Release 先執行 additive preserve-data `prepare`，再執行不帶 `--allow-destructive` 的 plain `sync`；若 plan 含 destructive / data-sensitive DDL，Automatic Release 必須 fail closed，改由 manual `Supabase Replace` 取得 explicit authorization。若 schema 未變，Release 仍執行 `verify`，避免 skipped job 被誤認成 remote convergence evidence。兩條 automatic path 都必須對 exact project 完成 post-operation readback，且 migration history before / after 保持一致；只有同一 validated revision 的 Supabase convergence 成功後才允許 Production deployment。
