...
- Release routing 必須維持：affected `supabase/schemas/*.sql` → `schema:remote sync`；schema unchanged → `schema:remote verify`；Supabase 成功後才允許 exact validated SHA 的 Vercel Production；Rich Menu 再依 affected source 於 deployment 成功後處理。
